...
export async function POST(req) {
    try {
        ...
        const senderObjectId = typeof user.id === 'string' ? new mongoose.Types.ObjectId(user.id) : user.id;
...
export async function GET(req) {
    try {
        ...
        if (!projectId && !url.searchParams.get('receiverId')) {
            return NextResponse.json({ error: 'Project ID or Receiver ID required' }, { status: 400 });
        }

        let query = {};
        // Only filter by project if specfically requested
        if (projectId) query.project = projectId;

        const receiverId = url.searchParams.get('receiverId');
        console.log('[GET Messages] User ID:', user.id, 'Type:', typeof user.id);
        console.log('[GET Messages] Receiver ID:', receiverId, 'Type:', typeof receiverId);
        console.log('[GET Messages] Project ID:', projectId);

        if (receiverId) {
            // Convert string IDs to ObjectId for proper comparison
            const mongoose = require('mongoose');
            const userObjectId = typeof user.id === 'string' ? new mongoose.Types.ObjectId(user.id) : user.id;
            const receiverObjectId = typeof receiverId === 'string' ? new mongoose.Types.ObjectId(receiverId) : receiverId;

            // Find messages between these two users (sent or received)
            // AND match project if specified
            // Using $and to combine potential project filter with user filter
            const userFilter = {
                $or: [
            ...
            // If no receiver, stick to project only (group chat logic, though we moved to 1-on-1)
            // But existing code used query.project = projectId.
        }
        ...
        if (messages.length > 0) {
            console.log('[GET Messages] First message - Sender:', messages[0].sender?._id, 'Receiver:', messages[0].receiver);
        }
