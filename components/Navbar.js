  ...
  return (
    <nav className="max-w-7xl mx-auto w-full flex justify-between items-center p-4 bg-[#F0F0F0]">
      ...
      <div className="text-xl font-bold">
        <Link href="/">
          <Image src={companyLogo} alt="Company Logo" className="h-8 w-auto" />
        </Link>
      </div>
        ...
        <div className="absolute top-16 right-4 w-48 bg-white shadow-lg rounded-lg border border-gray-100 p-2 z-50">
          ...
          <button
            onClick={() => {
              ...
              // Close dropdown (example)
              setMenuOpen(false);

              // Call the existing logout function
              logout();
            }}
            className="block w-full text-left px-4 py-2 hover:bg-gray-50 rounded text-red-600"
          >
            Logout
          </button>
        </div>
