...
export default function ProfileModal({ user, onClose }) {
  ...
  // 🔥 FILTER PROJECTS BASED ON PROFILE USER (NOT LOGGED-IN USER)
  const filteredProjects = projects.filter((project) => {
    // If viewing CLIENT profile → show client projects
    if (user.role === "client") {
      return project.client?._id === user._id;
    }

    // If viewing CONTRACTOR profile → show assigned projects
    if (user.role === "contractor") {
      return project.assignedContractors?.some((c) => c._id === user._id);
    }
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 relative">
        ...
        <div className="mt-6 space-y-4">
          ...
          {/* PROJECTS SECTION — DESIGN UNCHANGED */}
          <div>
            <h4 className="text-sm font-bold text-gray-700">
              {user.role === "contractor" ? "Projects Assigned" : "Projects"}
            </h4>
